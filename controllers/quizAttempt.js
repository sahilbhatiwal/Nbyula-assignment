...
exports.getNextQuestion = BigPromise(async (req, res, next) => {
    ...
    const { id} = req.params;
    const quizAttempt = await QuizAttemptModel.findById(id).populate("Quiz", "Questions");
    if (!quizAttempt) {
...
exports.submitQuestion = BigPromise(async (req, res, next) => {
    ...
    const questionAlreadyAttempted = AttemptedQuestions.some((attemptedQuestion) => {
